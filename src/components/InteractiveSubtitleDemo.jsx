...
const cues = [
  ...
  },
  {
    ...
    english: 'Only during Rika full manifestation can Okkotsu use high-output cursed energy.',
    parts: [
      { text: '乙骨', key: '乙骨' },
      { text: 'は', key: 'は' },
      { text: ' ' },
...
export default function InteractiveSubtitleDemo({ colors }) {
  ...
  const showCard = (event, key) => {
    ...
    setActiveCard({
      ...
      left: Math.max(16, Math.min(desiredLeft, frameRect.width - cardWidth - 16)),
        ...
        {activeCard ? (
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-2">
              <div>
                <div className="text-3xl font-black leading-none">{activeCard.key}</div>
                ...
                {demoCue.parts.map((part, index) =>
                  part.key ? (
                    <span
                      ...
                      style={{
                        backgroundColor:
                          activeCard?.key === part.key ? 'rgba(249,214,72,0.28)' : 'transparent'
                      }}
